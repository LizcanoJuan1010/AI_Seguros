...
from backend.models import Lead, Vehicle, ConsultationSlot, Appointment

logger = logging.getLogger(__name__)

# Asesores licenciados que atienden las consultas
LICENSED_AGENTS = [
    "Carolina Restrepo",
    "Andrés Gómez",
    "Juliana Torres",
...
def build_default_lead(phone: str) -> Lead:
    """Lead mock por defecto (demo de seguro de carro) con el teléfono dado.

    Cualquiera puede pasar su propio número y obtener una llamada de demo
    realista sin tocar el resto de la config."""
    return Lead(
        lead_id=f"lead_{uuid.uuid4().hex[:6]}",
        first_name="Camilo",
        last_name="Ramírez",
        phone=phone,
        email="camilo.ramirez@email.com",
        vehicle=Vehicle(
            brand="Mazda",
            model="3",
            year=2020,
        ...
        desired_coverage_start="2026-08-15",
        quote_submitted_at="2026-07-20T14:32:00Z",
...
def build_lead_from_dict(phone: str, data: dict) -> Lead:
    ...
    return Lead(
        lead_id=f"lead_{uuid.uuid4().hex[:6]}",
        first_name=data.get("first_name", "Camilo"),
        last_name=data.get("last_name", "Ramírez"),
        phone=phone,
        email=data.get("email", "cliente@email.com"),
        vehicle=Vehicle(
            brand=vehicle_data.get("brand", "Mazda"),
            model=vehicle_data.get("model", "3"),
            year=vehicle_data.get("year", 2020),
            city=vehicle_data.get("city", "Bogotá"),
        ),
        current_insurance_status=data.get("current_insurance_status", "switching"),
        desired_coverage_start=data.get("desired_coverage_start", "2026-08-15"),
