...
def _build_voicemail_text(lead_context: dict) -> str:
    """Mensaje de buzón personalizado con los datos del lead.

    Menos de 20 segundos hablado — breve y profesional."""
    first_name = lead_context.get("first_name", "")
    saludo = f"Hola {first_name}," if first_name else "Hola,"
    return (
        f"{saludo} te habla un asistente automático de Tequendama Seguros. "
        f"Te llamamos por la cotización de seguro de vehículo que solicitaste "
        f"recientemente. Nos encantaría conectarte con uno de nuestros asesores "
        f"licenciados para revisar tus opciones. Te volveremos a llamar pronto, "
        f"o puedes visitar nuestra página web cuando te quede mejor. "
        f"Gracias, y que tengas un buen día."
    )
...
async def deliver_voicemail(
    ...
    try:
        ...
        await lead_service.update_lead(
            ...
            call_summary=(
                f"La llamada a {first_name} {last_name} cayó en buzón de voz. "
                f"Se dejó un mensaje automático referenciando su cotización de "
                f"seguro de vehículo con Tequendama Seguros."
            ),
        )
    except Exception as e:
