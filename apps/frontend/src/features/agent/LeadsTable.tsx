...
export function LeadsTable({ leads, onSelect }: Props) {
  return (
    <div className="flex-1 overflow-auto bg-surface-container-low px-4 py-4 md:px-8 md:py-6">
      {/* Móvil (<md): cada lead como tarjeta apilada, sin scroll horizontal */}
      <div className="flex flex-col gap-3 md:hidden">
        {leads.map((lead) => (
          <div
            ...
            className="cursor-pointer rounded-2xl border border-outline-variant bg-surface-container-lowest p-4 shadow-sm transition-colors hover:bg-surface-container-low"
          >
            <div className="flex items-start gap-3">
              <div className="flex size-9 shrink-0 items-center justify-center rounded-full bg-primary-fixed text-xs font-bold text-primary uppercase">
                {lead.initials}
              </div>
              <div className="min-w-0 flex-1">
                <div className="flex items-start justify-between gap-2">
                  ...
                  <Chip tone={intentTone[lead.intent]} className="shrink-0">
                    <Icon
                      name={
                        lead.intent === 'hot'
            ...
            <Button
              className="mt-3 w-full justify-center rounded-md bg-primary-container py-2.5 text-on-primary-container"
              onClick={(e) => {
                e.stopPropagation()
                onSelect(lead.id)
              }}
      ...
      {/* Desktop (md+): tabla con scroll horizontal de respaldo */}
